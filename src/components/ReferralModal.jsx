...
const MOCK_NOTIFICATIONS = [
    { id: 1, type: 'signup',  name: 'Rahul M.', time: '2 min ago',  read: false },
    { id: 2, type: 'login',   name: 'Priya S.', time: '1 hr ago',   read: false },
    { id: 3, type: 'signup',  name: 'Amit K.',  time: '3 hrs ago',  read: true  },
    { id: 4, type: 'login',   name: 'Sneha R.', time: 'Yesterday',  read: true  },
];
    ...
    return (
        ...
        >
            <div style={{ padding: '1rem 1.25rem 0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #F1F5F9' }}>
                ...
                <button onClick={markAllRead} style={{ fontSize: '0.72rem', fontWeight: 700, color: '#1B6B3A', background: 'none', border: 'none', cursor: 'pointer' }}>Mark all read</button>
            </div>
                    ...
                    <div key={n.id} style={{ padding: '0.875rem 1.25rem', display: 'flex', gap: '0.75rem', alignItems: 'flex-start', background: n.read ? 'transparent' : '#F0FDF4', borderBottom: '1px solid #F8FAFC' }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: '0.78rem', color: '#64748B', marginTop: 2 }}>
                                <span style={{ color: '#1B6B3A', fontWeight: 600 }}>{n.name}</span> {n.type === 'signup' ? 'signed up. +500 pts pending.' : 'logged in. +500 pts credited!'}
                            </div>
...
/* ─── Tracking tab ───────────────────────────────────────────── */
const MOCK_HISTORY = [
    { id: 1, name: 'Rahul M.',  status: 'logged_in', pts: 500,  date: 'Today, 10:42 AM' },
    { id: 2, name: 'Priya S.',  status: 'signed_up', pts: 0,    date: 'Today, 09:15 AM' },
    { id: 3, name: 'Amit K.',   status: 'logged_in', pts: 500,  date: 'Yesterday' },
    { id: 4, name: 'Sneha R.',  status: 'logged_in', pts: 500,  date: '2 days ago' },
    { id: 5, name: 'Vikram T.', status: 'signed_up', pts: 0,    date: '3 days ago' },
];
                ...
                return (
                    <div key={h.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 0.875rem', background: '#F8FAFC', borderRadius: '12px', border: '1px solid #F1F5F9' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1E293B' }}>{h.name}</div>
                        ...
                        {h.pts > 0 && <span style={{ fontSize: '0.78rem', fontWeight: 800, color: '#F59E0B', whiteSpace: 'nowrap' }}>+{h.pts} pts</span>}
                    </div>
                );
            })}
        </div>
...
const ReferralModal = ({ isOpen, onClose }) => {
    ...
    const [activeTab, setActiveTab] = useState('refer');  // 'refer' | 'tracking'
    ...
    const stats = referralData?.stats ?? { referred: 4, converted: 3, pointsEarned: 1500 };

    useEffect(() => {
        if (!isOpen) { setCopied(false); setShareError(''); setShowNotifPanel(false); setActiveTab('refer'); }
    }, [isOpen]);
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ background: 'linear-gradient(135deg, #064E3B 0%, #1B6B3A 100%)', padding: '1.75rem 2.5rem 2rem', borderRadius: '32px 32px 0 0', position: 'relative', color: '#fff', overflow: 'hidden' }}>
                            ...
                            {/* Header row: bell | tracking | close */}
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', position: 'relative', zIndex: 2 }}>
                                ...
                                <div style={{ position: 'relative' }}>
                                    ...
                                    {unreadCount > 0 && (
                                        <span style={{ position: 'absolute', top: -4, right: -4, background: '#EF4444', color: '#fff', borderRadius: '50%', width: 16, height: 16, fontSize: '0.6rem', fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            {unreadCount}
                                        </span>
                                    )}
                                    <AnimatePresence>{showNotifPanel && <NotificationPanel onClose={() => setShowNotifPanel(false)} />}</AnimatePresence>
                                </div>

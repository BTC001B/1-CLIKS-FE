...
const BusinessCompare = () => {
    ...
    return (
        <div style={{ padding: '2.5rem', background: '#F0F9F4', minHeight: '100vh', fontFamily: "'Inter', sans-serif" }}>
            ...
            {/* Periodic Stats Snapshot - Matching Billing UI */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem', marginBottom: '3rem' }}>
                {[
                    { label: 'Current Revenue', value: `₹${(periodic?.current.revenue || 0).toLocaleString()}`, icon: TrendingUp, color: '#1B6B3A', bg: '#F0FDF4' },
                    { label: 'Target Achievement', value: '92%', icon: Target, color: '#0D9488', bg: '#F0FDFA' },
                    { label: 'Expense Ratio', value: '42%', icon: Activity, color: '#064E3B', bg: '#ECFDF5' },
                    { label: 'Growth Index', value: `+${getGrowth(periodic?.current.revenue, periodic?.previous.revenue).toFixed(1)}%`, icon: Zap, color: '#1B6B3A', bg: '#F0FDF4' }
                ].map((stat, idx) => (
